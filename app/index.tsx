...
export default function Index() {
  ...
  const renderItem = ({ item }: { item: BookRow }) => {
    const color =
      item.status === "planning" ? "#6b7280" : item.status === "reading" ? "#f59e0b" : "#10b981";
    return (
      <Pressable
      ...
      >
        <View style={{ flex: 1 }}>
          <Text style={[styles.bookTitle, item.status === "done" && styles.doneTitle]}>
            {item.title}
          </Text>
    ...
    <SafeAreaView style={styles.container}>
      ...
      <View style={styles.row}>
        ...
        <View style={styles.filterRow}>
          {(["all", "planning", "reading", "done"] as Array<BookStatus | "all">).map((s) => (
            <TouchableOpacity
              key={s}
            ...
            >
              <Text style={filterStatus === s ? styles.filterTextActive : styles.filterText}>
                {s === "all" ? "Tất cả" : s}
              </Text>
            </TouchableOpacity>
          ))}
      ...
      >
        <Text style={styles.importText}>{importing ? "Đang import..." : "Import từ API"}</Text>
      </TouchableOpacity>
          ...
          <View style={styles.modalContent}>
            ...
            <View style={{ flexDirection: "row", justifyContent: "space-between", marginBottom: 12 }}>
              {(["planning", "reading", "done"] as BookStatus[]).map((s) => (
                ...
                >
                  <Text style={editing?.status === s ? styles.statusTextActive : styles.statusText}>
                    {s}
                  </Text>
                </TouchableOpacity>
              ))}
            ...
            <TouchableOpacity
              onPress={async () => {
                if (!editing) return;
                await editBook(editing.id, { title: titleInput, author: authorInput || null, status: editing.status });
                setEditModalVisible(false);
                setEditing(null);
              }}
              style={styles.modalSave}
...
const styles = StyleSheet.create({
  ...
  row: { marginBottom: 8 },
  input: { backgroundColor: "#fff", borderRadius: 10, padding: 10, borderWidth: 1, borderColor: "#e5e7eb", marginBottom: 8 },
  filterRow: { flexDirection: "row", marginTop: 4, flexWrap: "wrap" },
  filterBtn: { paddingVertical: 6, paddingHorizontal: 10, borderRadius: 8, borderWidth: 1, borderColor: "#e5e7eb", marginRight: 6, marginTop: 6 },
  ...
  importBtn: { backgroundColor: "#10b981", padding: 10, borderRadius: 10, marginBottom: 8 },
  ...
  bookItem: { padding: 14, backgroundColor: "#fff", borderRadius: 12, flexDirection: "row", alignItems: "center", marginBottom: 10 },
  ...
  modalInput: { backgroundColor: "#fff", borderWidth: 1, borderColor: "#e5e7eb", borderRadius: 8, padding: 10, marginBottom: 10 },
  modalSave: { backgroundColor: "#2563eb", padding: 12, borderRadius: 8, marginBottom: 8 },
  ...
  statusSelect: { paddingVertical: 8, paddingHorizontal: 12, borderRadius: 8, borderWidth: 1, borderColor: "#e5e7eb" },
